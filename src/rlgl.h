 ...
 *rendered in batch, additionally, rlDrawRenderBatchActive() could be called to
 ...
 *Bound by default to shader location: RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEIDS
 *       #define RL_DEFAULT_SHADER_ATTRIB_NAME_BONEWEIGHTS  "vertexBoneWeights"
 * // Bound by default to shader location:
 *RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEWEIGHTS #define
...
#define RL_DEFAULT_SHADER_ATTRIB_NAME_INSTANCE_TX                              \
  "instanceTransform" // Bound by default to shader location:
                      // RL_DEFAULT_SHADER_ATTRIB_NAME_INSTANCE_TX
#endif
...
#define RL_DEFAULT_SHADER_UNIFORM_NAME_BONE_MATRICES                           \
  "boneMatrices" // bone matrices
#endif
...
void rlLoadExtensions(void *loader) {
  ...
  // Get supported extensions list
  GLint numExt = 0;
  const char **extList = (const char **)RL_MALLOC(
  ...
  for (int i = 0; i < numExt; i++) {
        ...
        NULL) // Broad check for instanced_arrays
    {
      ...
      } else if (strcmp(extList[i], (const char *)"GL_EXT_instanced_arrays") ==
                 0) // EXT
      {
        glDrawArraysInstanced = (PFNGLDRAWARRAYSINSTANCEDEXTPROC)((
      ...
      } else if (strcmp(extList[i], (const char *)"GL_NV_instanced_arrays") ==
                 0) // NVIDIA GLES
      {
        glDrawArraysInstanced = (PFNGLDRAWARRAYSINSTANCEDEXTPROC)((
      ...
      if (strcmp(extList[i], (const char *)"GL_EXT_draw_instanced") == 0) {
        ...
        glDrawElementsInstanced = (PFNGLDRAWELEMENTSINSTANCEDEXTPROC)((
            rlglLoadProc)loader)("glDrawElementsInstancedEXT");
      } else if (strcmp(extList[i], (const char *)"GL_NV_draw_instanced") ==
                 0) {
        glDrawArraysInstanced = (PFNGLDRAWARRAYSINSTANCEDEXTPROC)((
            rlglLoadProc)loader)("glDrawArraysInstancedNV");
        glDrawElementsInstanced = (PFNGLDRAWELEMENTSINSTANCEDEXTPROC)((
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format,
  ...
  if (mipmapCount > 1) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL,
                    mipmapCount); // Required for user-defined mip count
  }
#endif
      ...
      if (data == NULL) {
        if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) {
          if ((format == RL_PIXELFORMAT_UNCOMPRESSED_R32) ||
            ...
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, mipmapLevel,
                         glInternalFormat, mipSize, mipSize, 0, glFormat,
                         glType, NULL);
        } else
        ...
        if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
          glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, mipmapLevel,
                       glInternalFormat, mipSize, mipSize, 0, glFormat, glType,
...
unsigned char *rlReadScreenPixels(int width, int height) {
  ...
  // NOTE: Alpha value has already been applied to RGB in framebuffer, we don't
  // need it!
  for (int y = height - 1; y >= height / 2; y--) {
    for (int x = 0; x < (width * 4); x += 4) {
      ...
      imgData[s + 3] = 255; // Set alpha component value to 255 (no trasparent
                            // image retrieval)
...
unsigned int rlCompileShader(const char *shaderCode, int type) {
  ...
  if (success == GL_FALSE) {
    ...
    case GL_COMPUTE_SHADER:
      TRACELOG(
          RL_LOG_WARNING,
          "SHADER: Compute shaders not enabled. Define GRAPHICS_API_OPENGL_43",
          shader);
      break;
#endif
    default:
...
unsigned int rlLoadShaderProgram(unsigned int vShaderId,
  ...
  glBindAttribLocation(program, RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD2,
                       RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD2);
  glBindAttribLocation(program, RL_DEFAULT_SHADER_ATTRIB_LOCATION_INSTANCE_TX,
                       RL_DEFAULT_SHADER_ATTRIB_NAME_INSTANCE_TX);

#ifdef RL_SUPPORT_MESH_GPU_SKINNING
  glBindAttribLocation(program, RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEIDS,
                       RL_DEFAULT_SHADER_ATTRIB_NAME_BONEIDS);
